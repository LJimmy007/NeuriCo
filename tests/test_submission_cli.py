...
def test_cli_same_title_across_providers_preserves_content_mounts_and_repo_metadata(tmp_path, monkeypatch):
    ...
    class FakeGitHubManager:
        ...
        def create_research_repo(self, *, idea_id, title, description, private, domain,
                                 provider, no_hash, hypothesis):
    ...
    for provider in ("claude", "gemini", "codex"):
        ...
        spec = {"idea": {
            ...
            "local_resources": {"datasets": [{"path": str(resource), "usage": "Evaluation data"}]},
        }}
        source = tmp_path / f"{provider}.yaml"
        source.write_text(yaml.safe_dump(spec), encoding="utf-8")
        monkeypatch.setattr(sys, "argv", ["submit", str(source), "--provider", provider, "--private"])

        submit.main()

        persisted = yaml.safe_load((workspace / f"research-{provider}" / "research.yaml").read_text())
    ...
    for provider, (idea_id, original, resource) in expected.items():
        ...
        assert stored["metadata"]["github_repo_url"] == f"https://github.example/test/research-{provider}"
